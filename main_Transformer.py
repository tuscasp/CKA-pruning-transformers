...
def fine_tuning(model, X_train, y_train, X_test, y_test):
    ...
    for ep in range(0, 200): ### MUDAR PRA 200
...
if __name__ == '__main__':
    ...
    model = template_architectures.Transformer(input_shape, projection_dim, num_heads, n_classes)
    model.compile(loss='categorical_crossentropy', optimizer='Adam', metrics=['accuracy'])
    model.fit(X_train, y_train, batch_size=256, verbose=1, epochs=200)

    acc = accuracy_score(np.argmax(model.predict(X_test, verbose=0), axis=1), np.argmax(y_test, axis=1))
    print("Acc Unpruned: {}".format(acc))
    statistics(model)
    func.save_model('./models/TransformerViT_UNPRUNED', model)



    criteria = cl.criteria(criterion_layer)
    layers_to_prune = rl.layers_to_prune(model)
    # print(layers_to_prune)

    iteration = 0
    while (len(layers_to_prune) > 1):
        # print(len(layers_to_prune))
        scores = criteria.scores(model, X_train, y_train, layers_to_prune)
        model = rl.rebuild_network(model, scores, p_layer)
        layers_to_prune = rl.layers_to_prune(model)

        model = fine_tuning(model, X_train, y_train, X_test, y_test)
        acc = accuracy_score(np.argmax(y_test, axis=1), np.argmax(model.predict(X_test, verbose=0), axis=1))
        print('Iteration {}: Accuracy [{:.4f}]'.format(iteration, acc), flush=True)
        statistics(model)
        func.save_model('./models/TransformerViT_{}_p[{}]_iteration[{}]'.format(criterion_layer, p_layer, iteration), model)
        iteration += 1
